...
int ClientThread::receive_request_to_talk(unsigned char* session_key) {
	int ret = 0;
	unsigned char* plaintext = nullptr;
	size_t plaintext_len = 0;
...
int ClientThread::send_message_to_client(unsigned char* clients_session_key, unsigned char* server_session_key) {
	string message;
	int ret;
...
int ClientThread::send_plaintext (const int socket, unsigned char* msg, const size_t msg_len, unsigned char* key)
{
	int ret;
...
int ClientThread::receive_plaintext (const int socket, unsigned char*& msg, size_t& msg_len, unsigned char* shared_key)
{
	long ret_long = -1;
